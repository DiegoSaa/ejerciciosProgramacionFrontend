...
const Desplegable = (props) => {
  ...
  return (
    ...
    <Dropdown className="btn btn-primary btn-sm" onSelect={toggle}>
      <Dropdown.Toggle className="btn btn-secundary btn-sm"><img src={opciones} alt="opciones" /></Dropdown.Toggle>


      <Dropdown.Menu> 
        {generos.map((genero, index) => (
          <Dropdown.Item id={index} key={index} eventKey={index} active={activos[index]}> {genero} </Dropdown.Item>
        ))}
